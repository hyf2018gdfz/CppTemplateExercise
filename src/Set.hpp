...
public:
  ...
  auto insert(const T& val) -> Iterator {
    auto tmp = tree_.insert(val);
    return Iterator(tmp.first);
  }
  auto insert(T&& val) -> Iterator {
    auto tmp = tree_.insert(std::move(val));
    return Iterator(tmp.first);
  }
